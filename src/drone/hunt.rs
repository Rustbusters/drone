...
impl RustBustersDrone {
    ...
    pub fn set_data(&self, data: &mut [u8; FRAGMENT_DSIZE], hunt_mode: HuntMode) {
        match hunt_mode {
            HuntMode::NormalShot(src_node_id, target_drone_id) => {
                RustBustersDrone::set_normal_shot_data(data, src_node_id, target_drone_id)
            }
            HuntMode::LongShot(src_node_id, shot_range) => {
                RustBustersDrone::set_long_shot_data(data, src_node_id, shot_range)
            }
            HuntMode::EMPBlast(src_node_id) => {
                RustBustersDrone::set_emp_blast_data(data, src_node_id)
            }
        }
    }

    fn set_normal_shot_data(
        data: &mut [u8; FRAGMENT_DSIZE],
        src_node_id: NodeId,
        target_drone_id: NodeId,
    ) {
        data[0] = b'n'; // encoding of normal shot ('n' as byte)
        data[1] = src_node_id;
        data[2] = target_drone_id;
    }

    fn set_long_shot_data(
        data: &mut [u8; FRAGMENT_DSIZE],
        src_node_id: NodeId,
        shot_range: ShotRange,
    ) {
        data[0] = b'l'; // encoding of long shot ('l' as byte)
        data[1] = src_node_id;
        data[2] = shot_range;
    }

    fn set_emp_blast_data(data: &mut [u8; FRAGMENT_DSIZE], src_node_id: NodeId) {
        data[0] = b'e'; // encoding of emp blast ('e' as byte)
        data[1] = src_node_id;
    }
}
